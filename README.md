# 🎬 媒体刮削管理工具

可视化管理媒体文件，支持 TMDB 刮削和 AI 智能匹配的现代化 Web 应用。

## 📖 概念说明

| 术语 | 含义 | 何时使用 |
|------|------|----------|
| **刮削** | 首次处理媒体文件：匹配 TMDB → 创建 NFO → 下载海报 | 新入库的剧集/电影 |
| **补刮** | 检测并处理新增的文件（不影响已处理的） | 追剧时新下载了几集 |
| **刷新元数据** | 重新获取最新信息：更新 NFO + 下载海报/Fanart | 元数据有误或 TMDB 信息更新 |
| **重新匹配** | 手动选择 TMDB 条目（当自动匹配错误时） | 同名剧集/电影混淆 |
| **AI 消歧** | 当多个相似结果时，AI 自动选择最佳匹配 | 批量处理时自动完成 |

> 💡 **智能刮削**：程序会自动判断 —— 未刮削时执行"刮削"，已刮削时执行"补刮"。

## ✨ 功能特性

### 📥 收件箱
- 按目录分组查看待处理文件
- 自动解析文件名获取标题、季集、分辨率等信息
- TMDB 自动匹配和手动搜索
- 预览移动计划（目标路径、NFO 创建、覆盖提示）
- 批量刮削支持（手动匹配 / AI 自动）
- 一键入库：移动文件 + 生成 NFO + 下载海报

### 📺 剧集管理
- 表格视图展示剧集库
- 海报缩略图、TMDB ID、处理状态
- 多选支持（Shift/Ctrl 快捷键）
- 批量操作：刷新元数据、补刮、重新匹配
- 详情抽屉：剧集信息、季集列表、资产完整性
- 实时进度条和表格更新

### 🎬 电影管理
- 电影库表格视图
- 年份、TMDB ID 显示
- 批量刷新元数据
- 详情抽屉：电影信息、海报预览

### 🤖 AI 智能消歧
- 当 TMDB 返回多个相似结果时
- AI 根据文件名、年份、发行日期等自动选择最佳匹配
- 支持批量处理时自动消歧

### 📊 首页概览
- 媒体库统计：剧集数、集数、电影数、待处理数
- 最近任务列表

## 🚀 快速开始

### Docker Compose（推荐）

1. 创建配置文件：
```bash
cp .env.example .env
# 编辑 .env 设置媒体路径和 API 密钥
```

2. 启动服务：
```bash
docker compose up -d
```

3. 访问 http://localhost:3000

### 手动运行

需要 [Bun](https://bun.sh) 运行时（v1.0+）。

```bash
# 安装依赖
bun install
cd frontend && bun install && cd ..

# 构建前端
cd frontend && bun run build && cd ..

# 启动服务
bun run src/server.ts
```

## ⚙️ 配置

通过环境变量或 `.env` 文件配置：

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `INBOX_PATH` | 待处理文件目录 | `/mnt/media/Inbox` |
| `TV_PATH` | 剧集目录 | `/mnt/media/TV` |
| `MOVIES_PATH` | 电影目录 | `/mnt/media/Movies` |
| `TMDB_API_KEY` | TMDB API 密钥 | *必填* |
| `DIFY_API_KEY` | Dify AI API 密钥 | *可选* |
| `DIFY_URL` | Dify API 地址 | `https://api.dify.ai/v1/chat-messages` |
| `PORT` | 服务端口 | `3000` |

### TMDB API Key

1. 注册 [TMDB](https://www.themoviedb.org/) 账号
2. 进入 Settings > API > Create > Developer
3. 获取 API Key (v3 auth)

### Dify AI 消歧（可选）

> 玩 NAS 的该不会没装 [Dify](https://github.com/langgenius/dify) 吧？🤔

当 TMDB 搜索返回多个相似结果时，AI 消歧功能可以自动选择最佳匹配。

**配置步骤：**

1. 部署 Dify 实例（自托管或云服务）
2. 导入工作流：`dify/media-match-ranker.yml`
3. 发布应用并获取 API 密钥
4. 配置环境变量：
   - `DIFY_API_KEY` - 应用 API 密钥
   - `DIFY_URL` - API 地址，如 `https://your-dify.com/v1/chat-messages`

**工作流说明：**

- 接收包含 TMDB 候选的媒体文件信息
- AI 根据文件名、年份、发行日期等进行排序
- 返回最佳匹配的 TMDB ID 和置信度

## 📁 项目结构

```
media-scraper-web/
├── src/                    # 后端源码 (Bun + Hono)
│   ├── lib/
│   │   ├── config.ts       # 配置
│   │   ├── scanner.ts      # 文件扫描
│   │   ├── scraper.ts      # 刮削逻辑
│   │   ├── tmdb.ts         # TMDB API
│   │   ├── dify.ts         # AI 消歧
│   │   ├── tasks.ts        # 任务管理
│   │   └── progress.ts     # SSE 进度
│   ├── routes/
│   │   ├── media.ts        # 媒体 API
│   │   ├── scrape.ts       # 刮削 API
│   │   └── tasks.ts        # 任务 API
│   └── server.ts           # 入口
├── frontend/               # 前端源码 (SvelteKit)
│   └── src/
│       ├── lib/api.ts      # API 客户端
│       └── routes/         # 页面
├── docs/                   # 设计文档
│   ├── PRD.md             # 产品需求
│   ├── TAD.md             # 技术架构
│   └── UI.md              # UI 设计
├── docker-compose.yml
├── Dockerfile
└── .env.example
```

## 🔌 API 端点

### 媒体
- `GET /api/media/tv` - 剧集列表
- `GET /api/media/movies` - 电影列表
- `GET /api/media/inbox` - 收件箱文件
- `GET /api/media/inbox?view=dir` - 收件箱按目录分组
- `GET /api/media/stats` - 媒体库统计

### 刮削
- `GET /api/scrape/search/tv?q=` - 搜索剧集
- `GET /api/scrape/search/movie?q=` - 搜索电影
- `POST /api/scrape/match` - 自动匹配
- `POST /api/scrape/process/tv` - 处理剧集
- `POST /api/scrape/process/movie` - 处理电影
- `POST /api/scrape/refresh` - 刷新元数据
- `POST /api/scrape/supplement` - 补刮
- `POST /api/scrape/preview` - 预览移动计划
- `POST /api/scrape/batch` - 批量处理

### 任务（未完成）
- `GET /api/tasks` - 任务列表
- `POST /api/tasks/:id/cancel` - 取消任务

### 进度
- `GET /api/progress` - SSE 实时进度

## 🛠 开发

```bash
# 开发模式（后端）
bun run --watch src/server.ts

# 开发模式（前端）
cd frontend && bun run dev

# 运行测试
bun test
```

## 📝 License

MIT
